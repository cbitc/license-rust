...
- `LicenseEnvironment` 为**纯只读句柄**：`locate_default()`/`locate(path)` 只定位环境库、不做任何 I/O，读取时以 `SQLITE_OPEN_READ_ONLY` 打开，库文件不存在视为未激活。目录与表结构由激活入口独家创建，SDK 不写共享库。
...
```rust
use license_sdk::{LicenseError, LicenseEnvironment};

fn main() -> Result<(), Box<dyn std::error::Error>> {
    let environment = LicenseEnvironment::locate_default()?;
...
| 签名公钥 | SDK 内置信任公钥（kid `2026.08.05`）；`signing_keys` 表由激活入口维护，SDK 当前不读取 |

SDK **全程只读**：定位句柄不做 I/O，读取用只读连接，未激活时不会创建目录或库文件；校验失败的令牌不会被清除（清除是激活入口的策略），客户端可将 `Err` 一律视为未许可处理。
...
| 方法 | 说明 |
| --- | --- |
| `locate_default()` | 定位默认共享环境库（license-active 使用的路径）；不做任何 I/O，路径无法确定时报 `Environment` 错误。 |
| `locate(path)` | 定位指定路径的环境库（测试或定制部署用）；不做任何 I/O。 |
| `load_activation()` | 以只读连接读取激活记录 `Option<StoredLicense>`（令牌本体 + 来源 + 激活时间）；库文件不存在视为未激活。 |
...
高层入口：读取环境令牌 → 计算当前设备指纹 → 用内置公钥验签并校验 claims。
...
校验原语：对给定的紧凑 JWS 用指定公钥验签并校验 v3 claims。时间由调用方注入（便于测试），适合需要自定义密钥来源/时钟的场景。

### 其它

| 项 | 说明 |
| --- | --- |
| `get_environment_id()` | 计算当前环境指纹，与激活入口、服务端令牌中的 `fingerprintSha256` 同格式。 |
...
| 错误 | 含义 |
| --- | --- |
| `Environment(message)` | 无法定位或打开本地环境库（数据目录、数据库文件）。 |
| `Storage(message)` | 本地环境库读取失败。 |
